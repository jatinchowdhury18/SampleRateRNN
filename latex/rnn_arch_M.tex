...
\begin{tikzpicture}[node distance=1.5cm, background rectangle/.style={fill=white}, show background rectangle]
    ...
    \node (z1) [delay, below of=h0, yshift=0.5cm] {$z^{-M}$};
    \coordinate[below of=rlayer, yshift=0.5cm] (h1);
    \node (h1Label) [below of=h1, yshift=1.0cm, xshift=-0.4cm] {Previous State $\mathbf{h}[n-M]$};
    \node (MLabel) [left of=h1Label, xshift=-2.5cm] {$M = \lfloor \frac{F_{target}}{F_{training}} \rfloor$};
